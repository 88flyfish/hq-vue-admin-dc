      ...
      <div slot="bottom" class="bottom">
        ...
        <div class="titleBox">
          ...
          <el-button
            ...
            type="primary"
            icon="el-icon-plus"
            @click="add(item,index)">添加字段匹配</el-button>
        </div>
        <el-table
...
export default {
  ...
  watch: {
    'baseInfo.datasourceId'() {
      //   if (this.baseInfo.serviceType !== '2') return
      //   this.getTableList()
    },
    'serviceMetadataArr': {
  ...
  methods: {
    ...
    add(item, index) {
      ...
      this.fieldListTmp = item.fieldList.map(i => {
        const idx = item.matchTable.findIndex(j => {
    ...
    confirmMatch() {
      ...
      this._getModelField(this.reqModelTree)
      this.formTmp.autoGenerate = this.operationModelItem.autoGenerate
      this.formTmp.generateType = this.operationModelItem.generateType
    ...
    _getModelField(tree) {
      tree.some((item) => {
        if (this.formTmp.columnKey === item.columnKey) {
